...
from models_diffusers.unet_spatio_temporal_condition_controlnet2 import UNetSpatioTemporalConditionControlNetModel
from pipelines.pipeline_stable_video_diffusion_controlnet2 import StableVideoDiffusionPipelineControlNet
from models_diffusers.controlnet_sdv2 import ControlNetSDVModel
...
def main(args):
    ...
    unet = UNetSpatioTemporalConditionControlNetModel.from_pretrained(os.path.join(model_dir, "unet"),
        #torch_dtype=weight_dtype,
        low_cpu_mem_usage=True,
        custom_resume=True)

    # change channel9 to 8
    in_channels = 8
    out_channels = unet.conv_in.out_channels
    unet.register_to_config(in_channels=in_channels)
    with torch.no_grad():
        # original unet size = channel 8
        # get only front 8
        new_conv_in = nn.Conv2d(in_channels, out_channels, unet.conv_in.kernel_size, unet.conv_in.stride, unet.conv_in.padding)
        new_conv_in.weight.zero_()
        new_conv_in.weight[:, :8, :, :] = unet.conv_in.weight[:, :8, :, :].clone()
        unet.conv_in = new_conv_in


    controlnet = ControlNetSDVModel.from_pretrained(os.path.join(model_dir, "controlnet"),
        #torch_dtype=weight_dtype,
        low_cpu_mem_usage=True,
        custom_resume=True)
    ...
    with torch.no_grad():
        ...
        new_conv_in.weight.zero_()
        new_conv_in.weight[:, :8, :, :] = controlnet.conv_in.weight[:, :8, :, :].clone()
        controlnet.conv_in = new_conv_in

    conditioning_channels = 1
    out_channels = controlnet.controlnet_cond_embedding.conv_in.out_channels
    controlnet.register_to_config(conditioning_channels=conditioning_channels)
    with torch.no_grad():
        new_conv_in = nn.Conv2d(in_channels, out_channels,
                                controlnet.controlnet_cond_embedding.conv_in.kernel_size,
                                controlnet.controlnet_cond_embedding.conv_in.stride,
                                controlnet.controlnet_cond_embedding.conv_in.padding)
        new_conv_in.weight.zero_()
        new_conv_in.weight[:,:1,:,:] = controlnet.controlnet_cond_embedding.conv_in.weight[:,:1,:,:].clone()
    ...
    # controlnet의 requires_grad=True 파라미터도 추가
    ...
    class DummyVideoDataset(Dataset):
        ...
        def __init__(self,
            ...
            self.video_first_frame_path = f"/workspace/data/diffusion/Framer/TrainDataNerf/frame_00"
            self.depth_folder = f"/workspace/data/diffusion/Framer/TrainDataNerf/frame_00_depth"
            images = os.listdir(self.video_first_frame_path)
            self.datas = images
        ...
        def __getitem__(self, idx):
            ...
            # [1] final video
            data_name = self.datas[idx]
            first_folder = self.video_first_frame_path
            folder_base = os.path.split(first_folder)[0]
            pixel_values = torch.empty((self.sample_n_frames, 3, self.height, self.width), dtype=torch.float32)
            for i in range(self.sample_n_frames):
                folder_name = f'frame_{str(i).zfill(2)}'
                folder_path = os.path.join(folder_base, folder_name)
                frame_path =  os.path.join(folder_path, data_name)
                with Image.open(frame_path) as img:
                    img = img.convert('RGB')
                    img = img.resize((self.width, self.height))
                    img_tensor = torch.from_numpy(np.array(img)).float()  # (H, W, C)
                    img_tensor = img_tensor.permute(2, 0, 1)  # (C, H, W)
                    img_tensor = img_tensor / 127.5 - 1.0  # Normalize to [-1, 1]
                    pixel_values[i] = img_tensor

            # [2] first image depthmap
            ...
            numpy_depth_images = np.array(pil_image_to_numpy(Image.open(os.path.join(self.depth_folder, data_name))))
            depth_pixel_values = numpy_to_pt(numpy_depth_images)

            # Dummy motion value: scalar float
            motion_values = torch.rand(1).item()  # between 0.0 and 1.0
    ...
    def _get_add_time_ids(
        ...
        passed_add_embed_dim = addition_time_embed_dim * add_time_ids.size(1)
        # expected_add_embed_dim = unet.add_embedding.linear_1.in_features
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(controlnet):
                # --------------------------------------------------------------------------------------------------------------
                # 1. We want to learn the denoising process w.r.t the edited images which
                # are conditioned on the original image (which was edited) and the edit instruction.
                # So, first, convert images to latent space.
                # --------------------------------------------------------------------------------------------------------------
                pixel_values = batch["pixel_values"].to(weight_dtype).to(accelerator.device, non_blocking=True) # batch, frame_num, 3, H, W
                latents = tensor_to_vae_latent(pixel_values, vae) # batch, 14, 3, h, w
                ...
                inp_noisy_latents = noisy_latents / ((sigmas_reshaped ** 2 + 1) ** 0.5)  # [batch, 14, 4, h, w]
                # --------------------------------------------------------------------------------------------------------------
                # 2. conditional latents : first frame condition
                # --------------------------------------------------------------------------------------------------------------
                conditional_latents = small_noise_latents[:, 0, :, :, :] # [batch, 1, 4, h, w]
                conditional_latents = conditional_latents / vae.config.scaling_factor
                timesteps = torch.Tensor([0.25 * sigma.log() for sigma in sigmas]).to(latents.device)

                # --------------------------------------------------------------------------------------------------------------
                # 3. Get the text embedding for conditioning.
                # --------------------------------------------------------------------------------------------------------------
                encoder_hidden_states = encode_image(pixel_values[:, 0, :, :, :]) # first frame encode image

                # --------------------------------------------------------------------------------------------------------------
                # 4. added time ids
                # --------------------------------------------------------------------------------------------------------------
                added_time_ids = _get_add_time_ids(6,
                                                   100,
                                                   train_noise_aug, # noise_aug_strength == 0.0
                                                   encoder_hidden_states.dtype,
                                                   bsz,
                                                   unet, device=latents.device)
                ...
                conditional_latents = conditional_latents.unsqueeze(1).repeat(1, noisy_latents.shape[1], 1, 1, 1) # batch,frame_num,4,h,w
                inp_noisy_latents = torch.cat([inp_noisy_latents, conditional_latents], dim=2)                    # batch,frame_num,8,h,w
                controlnet_image = batch["depth_pixel_values"] # batch,frame_num,1,H,W

                target = latents
                down_block_res_samples, mid_block_res_sample = controlnet(inp_noisy_latents,
                                                                          timesteps,
                                                                          encoder_hidden_states,
                                                                          added_time_ids=added_time_ids,
                                                                          controlnet_cond=controlnet_image, # channel = 1
                                                                          return_dict=False,)
                # Predict the noise residual
                model_pred = unet(inp_noisy_latents, timesteps, encoder_hidden_states,
                    added_time_ids=added_time_ids,
                ...
                if accelerator.is_main_process:
                    ...
                    if (
                        ...
                        with torch.autocast(str(accelerator.device).replace(":0", ""), enabled=accelerator.mixed_precision == "fp16"):
                            ...
                            validation_image = Image.open(f"/workspace/data/diffusion/Framer/TrainDataNerf/frame_00/rgb-273.png").convert('RGB')
                            validation_control_images = Image.open(f"/workspace/data/diffusion/Framer/TrainDataNerf/frame_00_depth/rgb-273.png").convert('L')


                            video_frames = pipeline(image = validation_image,
                                                    controlnet_condition = validation_control_images,
                                                    height=args.height,
                                                    width=args.width,
