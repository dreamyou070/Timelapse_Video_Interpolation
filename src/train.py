...
def main():
    ...
    unet = UNetSpatioTemporalConditionModel.from_pretrained(os.path.join("./checkpoints/framer_512x320", "unet"),
                                                            torch_dtype=dtype,
                                                            low_cpu_mem_usage=True,
                                                            variant = "fp16",
                                                            # low_cpu_mem_usage=False,
                                                            custom_resume=True, ).to(device, dtype)
    controlnet = ControlNetSVDModel.from_pretrained(os.path.join("./checkpoints/framer_512x320", "controlnet")).to(device, dtype)
