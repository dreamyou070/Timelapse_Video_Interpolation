...
from models.utils import generate_gassian_heatmap
from models.unet_spatio_temporal_condition import UNetSpatioTemporalConditionModel
from einops import rearrange
from models.sift_match import point_tracking, interpolate_trajectory
...
@dataclass
class StableVideoDiffusionEfficientInterpPipelineOutput(BaseOutput):
    frames: Union[List[PIL.Image.Image], np.ndarray]


class StableVideoDiffusionEfficientInterpPipeline(DiffusionPipeline):
    ...
    def __init__(
        ...
        # self.image_encoder = CLIPVisionModelWithProjection
    ...
    def _encode_image(self, image, device, num_videos_per_prompt, do_classifier_free_guidance):
        ...
        if do_classifier_free_guidance:
            ...
            image_embeddings = torch.cat([negative_image_embeddings, image_embeddings]) # [batch,1,1024]
    ...
    def __call__(
        ...
        last_channel_lastframe_condition=False,
        seoncd_channel_firstframe_condition=False,
        channelwise_all_front=False,
        channelwise_all_end=False,
        projector = None,
        do_motion_prompt = False,):
        ...
        # ------------------------------------------------------------------------------------------------------------------------------------------------------------------------
        image_embeddings = self._encode_image(image, device, num_videos_per_prompt, do_classifier_free_guidance)        # uncon[1,1,1024] + cond[1,1,1024]
        image_end_embeddings = self._encode_image(image_end, device, num_videos_per_prompt, do_classifier_free_guidance)# uncon[1,1,1024] + cond[1,1,1024]
        ...
        if do_motion_prompt :
            ...
            if projector is not None:
                image_embeddings = projector(image_end_embeddings)
                image_end_embeddings = projector(image_end_embeddings)
                image_embeddings = torch.cat([image_embeddings, image_end_embeddings], dim=1)  # [2,2,1024]
        ...
        dtype = image.dtype
        noise = randn_tensor(image.shape, generator=generator, device=image.device, dtype=dtype)
        image = image + noise_aug_strength * noise
        image_end = self.image_processor.preprocess(image_end, height=height, width=width)
        noise = randn_tensor(image_end.shape, generator=generator, device=image_end.device, dtype=dtype)
        ...
        image_end_latents = image_end_latents.unsqueeze(1) # [2,1,4,64,40]
        ...
        mask_channel[:, 0:1, :, :, :] = 0 # only condition be zero
        mask_channel[:, -1:, :, :, :] = 0
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                kwargs = {}
                outputs = self.unet(latent_model_input,
                                    t,
