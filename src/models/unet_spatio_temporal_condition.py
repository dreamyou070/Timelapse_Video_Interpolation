...
from models.attention_processor import CROSS_ATTENTION_PROCESSORS, AttentionProcessor, AttnProcessor
from diffusers.models.embeddings import TimestepEmbedding, Timesteps
from diffusers.models.modeling_utils import ModelMixin
from models.unet_3d_blocks import UNetMidBlockSpatioTemporal, get_down_block, get_up_block
import os
import re
...
class UNetSpatioTemporalConditionModel(ModelMixin, ConfigMixin, UNet2DConditionLoadersMixin):
    ...
    def forward(
        ...
        if isinstance(encoder_hidden_states, tuple):
            spatial_encoder_hidden_states, temporal_encoder_hidden_states = encoder_hidden_states
            spatial_encoder_hidden_states = spatial_encoder_hidden_states.to(emb.dtype).repeat_interleave(num_frames, dim=0)   # frame*batch / 2 / dimension
            temporal_encoder_hidden_states = temporal_encoder_hidden_states.to(emb.dtype).repeat_interleave(num_frames, dim=0) # frame*batch / 1 / dimension
            encoder_hidden_states = (spatial_encoder_hidden_states, temporal_encoder_hidden_states)
        else :
        ...
        # 4. mid
        sample = self.mid_block(hidden_states=sample,
            temb=emb,
            encoder_hidden_states=encoder_hidden_states,
            image_only_indicator=image_only_indicator,
        )
    ...
    def from_pretrained(cls, pretrained_model_name_or_path: Optional[Union[str, os.PathLike]],
                        custom_resume=False, **kwargs):
        print(f' from pretrained !!! ')
        r"""
        ...
        print(f'custom_resume = {custom_resume}')
        if not custom_resume:
            original_in_channels = config['in_channels']
            print(f'original_in_channels={original_in_channels}')
        ...
        print(f' set torch dtype of model .. ')
        ...
        print(f' register to cofig .. ')
        model.register_to_config(_name_or_path=pretrained_model_name_or_path)

        # Set model in evaluation mode to deactivate DropOut modules by default
        print(f' set model eval .. ')
        model.eval()
