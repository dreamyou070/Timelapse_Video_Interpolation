...
def get_vis_image(target_size=(512, 512), points=None, side=20, num_frames=14):
    ...
    for idxx, point in enumerate(trajectory_list[0]):
                ...
                if cc >= 0:
                    ...
                    if len(trajectory_) == 1:
                        ...
                        for itt in range(len(trajectory_) - 1):
                            cv2.line(vis_img, (trajectory_[itt][0], trajectory_[itt][1]),
                                     (trajectory_[itt + 1][0], trajectory_[itt + 1][1]), (255, 255, 255), 3)
        img = new_img
        ...
        pil_img.save(f'test_mask_{idxx}.png')
...
class Drag:
    ...
    @spaces.GPU
    def __init__(self, device, args, height, width, model_length, use_sift=False):

        logger.info(" ✅ step 1. load model")
        logger.info(" (1.1) unet")
        self.device = device
        ...
        args.model = "./checkpoints/framer_512x320"
        unet = UNetSpatioTemporalConditionModel.from_pretrained(os.path.join(args.model, "unet"),
                                                                torch_dtype=self.dtype,
                                                                low_cpu_mem_usage=True,
                                                                custom_resume=True, )
        unet = unet.to(device, self.dtype)
        for name, module in unet.named_modules():
            module.to(device=device, dtype=self.dtype)
        for name, param in unet.named_parameters(recurse=True):
            param.data = param.data.to(device=device, dtype=self.dtype)
        for name, buffer in unet.named_buffers(recurse=True):
            buffer.data = buffer.data.to(device=device, dtype=self.dtype)

        # 모델 다 돌면서 dtype 을 변경해주기


        logger.info(" (1.2) controlnet")
        controlnet = ControlNetSVDModel.from_pretrained(os.path.join(args.model, "controlnet"), )
        controlnet = controlnet.to(device, self.dtype)
        for name, module in controlnet.named_modules():
            module.to(device=device, dtype=self.dtype)
        for name, param in controlnet.named_parameters(recurse=True):
            param.data = param.data.to(device=device, dtype=self.dtype)
        for name, buffer in controlnet.named_buffers(recurse=True):
            buffer.data = buffer.data.to(device=device, dtype=self.dtype)
        ...
        logger.info(" ✅ step 2. set base configs")
        self.height = height
        self.width = width
        self.args = args
    ...
    def run(self, first_frame_path, last_frame_path, tracking_points, controlnet_cond_scale, motion_bucket_id):
        ...
        print(f' - input_all_points = {input_all_points}')
        # input_all_points = [[[start_x,start_y], [end_x, end_y]]] : x 는 오른쪽으로 갈 수록 커지고, y 는 위로 갈수록 작은값
        ...
        if (len(input_all_points) == 0) and self.use_sift:
            ...
            # axis resizing aligned with self.height, and self.width
        ...
        # ----------------------------------------------------------------------------------------------------------------
        vis_images = get_vis_image(target_size=(self.args.height, self.args.width),
                                   points=pred_tracks,
        ...
        print(f' (1) with_control = {with_control}')
        print(f' (2) pred_tracks = {pred_tracks}')
        print(f' (3) point_embedding = {point_embedding}') # None
        print(f' (4) sift_track_update = {sift_track_update}')
        print(f' (5) anchor_points_flag = {anchor_points_flag}')
        # sift_track_update = None (when there is no guidance)
        # anchor_points_flag = None
        # what is anchor point flag ???
        video_frames = self.pipeline(image,
                                     image_end,
                                     # trajectory control
                                     with_control=with_control,
                                     point_tracks=pred_tracks, # point tracks !!!!!!!!!!!
        ...
        for i, frame in enumerate(video_frames) :
            frame.save(os.path.join(args.output_dir, f'frame_{i}.png'))
        ...
        save_gifs_side_by_side(
            video_frames, # video
            vis_images[:self.model_length], # heat map
            val_save_dir,
    ...
    for track in tracking_points.constructor_args['value']:
        if len(track) > 1:
            for i in range(len(track) - 1):
                ...
                if i == len(track) - 2:
                    cv2.arrowedLine(transparent_layer, tuple(start_point), tuple(end_point), (255, 0, 0, 255), 2,
...
if __name__ == "__main__":
    ...
    with gr.Blocks() as demo:
        ...
        Framer = Drag("cuda", args, 320, 512, args.num_frames)
        ...
        image_upload_button.upload(preprocess_image, image_upload_button,
                                   [input_image, first_frame_path, tracking_points])

        image_end_upload_button.upload(preprocess_image_end, image_end_upload_button,
                                       [input_image_end, last_frame_path, tracking_points])

        add_drag_button.click(add_drag, tracking_points, [tracking_points, ])

        delete_last_drag_button.click(delete_last_drag, [tracking_points, first_frame_path, last_frame_path],
                                      [tracking_points, input_image, input_image_end])

        delete_last_step_button.click(delete_last_step, [tracking_points, first_frame_path, last_frame_path],
                                      [tracking_points, input_image, input_image_end])

        reset_button.click(reset_states, [first_frame_path, last_frame_path, tracking_points],
                           [first_frame_path, last_frame_path, tracking_points])

        input_image.select(add_tracking_points, [tracking_points, first_frame_path, last_frame_path],
                           [tracking_points, input_image, input_image_end])

        input_image_end.select(add_tracking_points, [tracking_points, first_frame_path, last_frame_path],
                               [tracking_points, input_image, input_image_end])

        run_button.click(Framer.run,
                         [first_frame_path, last_frame_path, tracking_points, controlnet_cond_scale, motion_bucket_id],
                         output_video)

    logger.info(f' ✅ step 0. start deo')
